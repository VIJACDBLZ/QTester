$( document ).ready(function() {
    ...
    QTcache.benchmarkTable = $('#benchmark-table').DataTable({
        "bInfo" : false,
        oLanguage: {
            sLengthMenu: "_MENU_",
        }
    ...
    fetch("/requestConfigs")
        .then(function(response) {
            if (!response.ok) {
                alert("Error!, Unable to contact server "+response.statusText);
                throw Error(response.statusText);
            }
            return response.json();
        }).then(function(response) {

            QTcache.requestConfigList  = response.requestConfigs;
            QTcache.requestConfigTitles = QTcache.requestConfigList.map(requestConfig => requestConfig["request-title"]);

            buildDropDown(QTcache.requestConfigTitles);
...
let search = document.getElementById("searchRequest");
